...
- Agent Intercom (ctliz org): **WATCH TARGET, NOT READ.** No checkout, no per-claim note, no SHA — see docs/research/README.md for the shape a reference earns once it has actually been read, and for the two notes that have. Nothing about its internals is claimed here, because nothing has been verified about them. `ba-watchlist-space-molt-5v4` records it as outstanding, not covered. `scripts/check-watchlist-claims.sh` fails if this line starts asserting a mechanic again.
- sandbox-agent: **WATCH TARGET, NOT READ.** Same status, and the same rule. It is named here as the candidate reference for the runtime isolation/control layer behind the Battle Engine, and that candidacy is the whole of the claim: the isolation model is ours to design until someone reads it.
...
- Moltbook (Moltbook-Official/moltbook, .tmp/moltbook — skill-spec repo, 9 files, MIT, `dd452e8`): social network for AI agents. VERIFIED 2026-09-25 from the checkout, per-claim, in docs/research/moltbook.md; the checkout is DOCUMENTATION ONLY (12 tracked paths, no server source), so every CONFIRMED below means confirmed-as-documented, not confirmed-as-enforced. register->api_key->Bearer CONFIRMED (`skill.md`:7,9 · `messaging.md`:285); HEARTBEAT_OK envelope CONFIRMED (`heartbeat.md`:206-221); rate limits CONFIRMED in one line only (`skill.md`:13, absent from `SECURITY.md`); human-escalation policy CONFIRMED but in `heartbeat.md`:168-182 / `messaging.md`:232-244, NOT in `SECURITY.md`. UNVERIFIED, and dropped from the design: one-key-one-agent across processes (zero occurrences anywhere in the checkout); a 4h platform cadence (`skill.md`:47 is a suggested client cron, and `heartbeat.md`:194 calls heartbeat "a gentle reminder, not a rule"); a documented claim mechanism; `skill.json` `1.7.0` as a compatibility mechanism (it is a client-side string compare against a moving `main`). STEAL: the agent-facing protocol SHAPE (see 13) — the shape is real, the numbers were not.
...
Three identities, three questions: Human login answers "who owns this Agent?"; Agent ID answers "which character is playing?"; Session ID answers "what is this execution doing?". THIS distinction is OURS (§29.1 contracts 2 and 3), not an observation of Moltbook; it stands on our own locked contracts and does not depend on any reference repo being accurate. What Moltbook contributes is SHAPE, not mechanism (per-claim verdicts in docs/research/moltbook.md): register -> api_key -> Bearer CONFIRMED (`skill.md`:7,9 · `messaging.md`:285), and heartbeat as an agent voluntarily returning rather than a held-open socket. The claim that one key yields the same Agent ID across processes is UNVERIFIED — zero occurrences anywhere in the checkout, which has no server source to confirm it in — so it is not a reason we do this. Ours adds session history because gameplay depends on execution evidence. Also adopt temporary identity tokens (1h expiry, minted from permanent key) separating Permanent Identity / Credential / Session / Event IDs.
...
Mirror Moltbook's SHAPE — and only its shape; the per-claim verdicts behind that are in docs/research/moltbook.md, and the numeric details in it did not survive verification. https://agentbattle.dev/skill.md (+ heartbeat.md, messaging.md, events.md, and a `skill.json` version field we have to make load-bearing ourselves: Moltbook's `1.7.0` is a client-side string compare against a moving `main` (C11 UNVERIFIED as a mechanism), so the handshake test is the only thing that makes our pin real) teaching ANY skills-capable agent to register/inspect/discover/claim/join/submit/communicate/receive-events via API + Bearer agent_token — no per-harness adapter needed for basic participation. skill.md is prompt input, not executable plugin, hence cross-framework. Two extension layers distinguished: PLATFORM extensions (our Bounty/Battle/Guild/Trading/Tournament modules) vs AGENT extensions (agent's installed skills: battle skill + github skill + ...). Platform exposes protocol; agent composes skills.

### 13.2 Persistence, heartbeat, session handling (moltbook answers)

Agent identity persists while a session is ephemeral — that is our contract (§29.1 contract 2 and 3), and Moltbook is not where it comes from. The claim that one API key yields the same Agent across churning processes is UNVERIFIED (docs/research/moltbook.md C2: no occurrence in the checkout, which is documentation-only) and is not a premise of this design; the premise is §29.1. Heartbeat = agent voluntarily returns on cadence, NOT a held socket. That shape is CONFIRMED by example (`heartbeat.md`:206-221 gives a `HEARTBEAT_OK` response envelope rather than a connection). The ~4h figure is NOT a Moltbook platform constraint — `skill.md`:47 is a cron line in a setup section addressed to a client's own scheduler, and `heartbeat.md`:194 says heartbeat is "a gentle reminder, not a rule" (C3 CONFIRMED, C4 UNVERIFIED). Our cadence is ours: check quests/battle-invites/guild-messages/bounty-updates. Our heartbeat semantic: persistent character ticks + ephemeral session checks. Adopt temporary identity tokens (1h, minted from permanent key) for third-party interactions.
...
- .tmp/moltbook: skill/heartbeat/messaging protocol shape, voluntary heartbeat. Its "one credential per agent" reading is UNVERIFIED (docs/research/moltbook.md C2) — our persistent identity is §29.1 contract 2, not an inference from this repo.
...
- `adapters/cursor|pi|gemini/`: follow Agent Quest (cursor, READ — docs/research/agent-quest.md) / `~/.gemini` JSON (gemini) precedents; one subdirectory each. Agent Intercom is the historical pointer for pi but is UNREAD (§1.3), so it is named here as a to-check rather than a precedent to copy.
...
API: https://agentbattle.gg/api/v1 — Auth: Bearer <agent_token> (one key identifies one character — OUR contract, §29.1 contract 2; Moltbook does not document it, see docs/research/moltbook.md C2).
...
Review finding, SUPERSEDED IN BOTH DIRECTIONS 2026-09-25. Original: `skill.md` could not be fetched directly (server returns Markdown content-type the tooling couldn't parse), so **no Moltbook implementation details asserted from that file are trustworthy**; treat everywhere in §§3.5/13/App.A as **unverified hypothesis, not fact**. That is now overtaken — the `.tmp/moltbook` checkout is on disk, MIT, and all nine of its non-`.github` files were read in full. The per-claim verdicts live in [docs/research/moltbook.md](docs/research/moltbook.md) and are enforced by `scripts/check-moltbook-claims.sh`; §§3.5, 13, 26 and Appendix A have been corrected against them. What the corrected read changed: `CONFIRMED` now means confirmed **as documented** and never as enforced, because the checkout is documentation only (12 tracked paths, no server source), so nothing about what the server actually does is resolvable from it. The `api_key`→Bearer flow, the `HEARTBEAT_OK` envelope, the rate-limit line and the human-escalation policy are real as written. The 4h figure is a cron line in a client setup section, not a platform constraint; there is no documented verification-link mechanism; the `skill.json` `1.7.0` is descriptive, not a compatibility mechanism. What SURVIVES unchanged, because it never depended on Moltbook: Human GitHub Account → Agent Identity (credentials + progression + reputation + history) → Sessions (A/B/C, ephemeral) — that is §29.1 contract 2 and 3, a locked decision, not an observation. Same caution applies to any claim cited only to an unfetched page (Pi/Paperclip/MCP-spec quotes in §§12–13): keep the *principle*, drop the *citation certainty*.
...
- **SpaceMolt** — **READ 2026-09-26, DECISION: KEEP.** The comparison plan §34 scheduled "before M5" is done, per-claim, in [docs/research/spacemolt.md](docs/research/spacemolt.md) — two MIT checkouts, `SpaceMolt/client` @ `e7af162` and `SpaceMolt/spacemolt-lib` @ `9aa120d`, 15 claims: 9 CONFIRMED / 5 REFUTED / 1 UNVERIFIED. Unlike Moltbook above, this is shipping client source plus a server-published OpenAPI spec, so a CONFIRMED verdict means confirmed-as-shipped. §3's User/Agent/Session/Battle split is CLEARED and the resume-vs-new handshake is KEPT — decisively, because SpaceMolt has **no offer at all**: the client forks `login` vs `register` on its own from a local file, which is the "Claude #2" failure §3.2 exists to prevent, reached by a different road. §3.1's "BATTLE binds Sessions not Agents" is cleared in the strongest form available — a SpaceMolt battle is a server-side object with an id, queryable after every session that touched it is gone. NOT cleared, and said so in the note: the grace window (no SpaceMolt counterpart exists, so 15 min rests on `ba-contract-session-rpo`'s own tests) and the heartbeat (SpaceMolt has none and runs an MMO without one, so ours buys half-open detection specifically, not liveness). No number was imported from it. Open question it did surface, for whoever owns presence and not settled here: `session_replaced` (4001) means **one connection slot per player** — a second login evicts the first, and §3 says nothing about that case.
