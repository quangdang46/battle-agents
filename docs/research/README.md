...
Six per-repo notes on what the reference implementations actually _do_, written so the
...
- **Cite the SHA, not the repo name.** All five non-learn-spine references are moving
...
`ba-adapters-template-a09`) were still unstarted. Re-check a note against its SHA before
relying on it; if the SHA no longer matches, the note is describing different code.
